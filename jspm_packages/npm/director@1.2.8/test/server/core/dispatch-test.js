...
vows.describe('director/core/dispatch').addBatch({"An instance of director.Router": {
    topic: function() {
      var that = this;
      that.matched = {};
      that.matched['/'] = [];
      ...
      var router = new director.Router({
        '/': {
          before: function() {
            that.matched['/'].push('before /');
          },
          on: function() {
            that.matched['/'].push('on /');
          },
          after: function() {
            that.matched['/'].push('after /');
          }
        },
        '/foo': {
          before: function() {
          ...
          on: function() {
            that.matched.foo.push('on foo');
          },
          after: function() {
            that.matched.foo.push('after foo');
          },
          '/bar': {
            before: function() {
              that.matched.foo.push('before foo bar');
            },
            on: function() {
              that.matched.foo.push('foo bar');
            },
            after: function() {
              that.matched.foo.push('after foo bar');
            },
            '/buzz': function() {
              that.matched.foo.push('foo bar buzz');
            }
          }
        },
        '/f*': {'/barbie': function() {
            that.matched['f*'].push('f* barbie');
          }}
      });
