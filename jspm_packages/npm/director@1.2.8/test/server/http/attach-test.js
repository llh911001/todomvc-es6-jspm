...
function assertData(uri) {
  return macros.assertGet(9091, uri, JSON.stringify([1, 2, 3]));
}
vows.describe('director/http/attach').addBatch({"An instance of director.http.Router": {"instantiated with a Routing table": {
      topic: new director.http.Router({'/hello': {get: handlers.respondWithData}}),
      ...
      "when passed to an http.Server instance": {
        topic: function(router) {
          ...
          helpers.createServer(router).listen(9091, this.callback);
        },
