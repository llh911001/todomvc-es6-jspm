...
function assertBark(uri) {
  return macros.assertGet(9090, uri, 'hello from (bark)');
}
vows.describe('director/http').addBatch({"An instance of director.http.Router": {"instantiated with a Routing table": {
      topic: new director.http.Router({'/hello': {get: handlers.respondWithId}}),
      ...
      "when passed to an http.Server instance": {
        topic: function(router) {
          ...
          helpers.createServer(router).listen(9090, this.callback);
        },
        ...
        "a request to /v2/somepath": macros.assertGet(9090, '/v2/somepath', 'hello from (/v2)')
      }
    }}}).export(module);
